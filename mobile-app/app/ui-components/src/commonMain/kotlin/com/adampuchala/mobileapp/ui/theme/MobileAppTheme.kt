...
val LocalColors = compositionLocalOf<Colors> {
    error("MobileAppTheme must be part of the call hierarchy to provide colors")
}

val LocalShapes = compositionLocalOf<Shapes> {
    error("MobileAppTheme must be part of the call hierarchy to provide dimensions")
}

val LocalTypography = compositionLocalOf<Typography> {
    error("MobileAppTheme must be part of the call hierarchy to provide typography")
}

object MobileAppTheme {
    val colors: Colors
        @Composable
        @ReadOnlyComposable
...
@Composable
fun MobileAppTheme(
    colors: Colors = if (isSystemInDarkTheme()) MobileAppDarkColors else MobileAppLightColors,
    rippleEnabled: Boolean = true,
    content: @Composable () -> Unit,
) {
    CompositionLocalProvider(
        LocalColors provides colors,
        LocalShapes provides MobileAppShapes,
        LocalTypography provides MobileAppTypography,
